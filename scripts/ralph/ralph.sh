...
is_retryable_error() {
  ...
  if grep -qE "No messages returned|ECONNRESET|ETIMEDOUT|rate limit|503|502|504|overloaded" <<< "$output"; then    
    return 0
  fi
  return 1
}
...
run_claude_with_stream() {
  ...
  claude -p "$prompt_content" --output-format stream-json --verbose 2>&1 > "$STREAM_OUTPUT" &
  local claude_pid=$!
...
run_with_retry() {
  ...
  while [ $attempt -le $MAX_RETRIES ]; do
    ...
    if [[ "$TOOL" == "amp" ]]; then
      output=$(cat "$SCRIPT_DIR/prompt.md" | amp 2>&1 | tee /dev/stderr) || exit_code=$?
    else
